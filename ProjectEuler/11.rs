fn main() {
    // addition of zeros, in this way makes the algorithm simpler to implement
    let grid = [
    ...
    // Iterating only over the numbers(not the added zeros)
    for i in 3..l {
        for j in 3..l {
            // Their are 4 unique directions,
            let right = grid[i][j] * grid[i][j+1] * grid[i][j+2] * grid[i][j+3];
            let down = grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j];
            let rowne = grid[i][j] * grid[i-1][j+1] * grid[i-2][j+2] * grid[i-3][j+3];
            let rowse = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3];
            max = if right>max {right} else {max};
            max = if down>max {down} else {max};
            max = if rowne>max {rowne} else {max};
            max = if rowse>max {rowse} else {max};
        }
    }
    println!("{}", max);
}
